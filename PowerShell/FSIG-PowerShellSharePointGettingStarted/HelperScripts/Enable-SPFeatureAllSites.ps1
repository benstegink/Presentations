...
Param
  ...
  elseif($Scope -eq "Web"){
    $webs = Get-SPWebApplication $WebAppUrl | Get-SPSite -Limit ALL | Get-SPWeb -Limit ALL | foreach($_.Url)
    foreach($web in $webs){
      if((Get-SPFeature -Identity $FeatureID -Web -ErrorAction SilentlyContinue) -eq $null)
      {
        Write-Host "Enable Feature on $_"
          Enable-SPFeature -Identity $FeatureID -URL $web
      }
    }
  }
  else{
